...
def plot_player_OE_bar(df: pd.DataFrame) -> plt.Figure:
    ...
    Grafica barras horizontales de OE con cuatro columnas:
      ...
      4) barra con el valor de OE

    Input df: columnas ['JUGADOR','DORSAL','IMAGEN','EQUIPO','OE']
    """
    ...
    # Normalizar valores OE para el colormap (0 = peor, 1 = mejor)
    oe_min, oe_max = df['OE'].min(), df['OE'].max()
    if oe_max > oe_min:
        df['OE_norm'] = (df['OE'] - oe_min) / (oe_max - oe_min)
    else:
        df['OE_norm'] = 0.5  # Si todos tienen el mismo valor
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        max_oe = df['OE'].max()
        text_color = 'black' if val < (max_oe * 0.2) else 'white'
...
if __name__ == "__main__":
    ...
    fig = plot_player_OE_bar(stats_df)
    fig.savefig("oe_demo.png", dpi=300)
