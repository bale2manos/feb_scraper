...
def plot_top_assists_vs_turnovers(
    df: pd.DataFrame,
    min_games: int = 5,
    min_minutes: int = 50
):
    """
    Assists vs Turnovers: scatter AST/game vs TOV/game, bubble size ~ minutes,
    color = team's main logo color, median reference lines.
    Shows the assist-to-turnover ratio visually.
    """
    # 0) load + Montserrat
    setup_montserrat_font()
    ...
    # 6) bubble size based on minutes played
    min_size = 200  # Minimum bubble size
    max_size = 1500  # Maximum bubble size
    
    # Normalize minutes values to bubble sizes
    if minutes.max() > minutes.min():
        minutes_normalized = (minutes - minutes.min()) / (minutes.max() - minutes.min())
        s = min_size + (max_size - min_size) * minutes_normalized
    else:
        s = np.full(len(minutes), (min_size + max_size) / 2)  # All same size if all minutes equal
    ...
    for i, (xi, yi, formatted_name, team) in enumerate(zip(x, y, formatted_names, teams)):
        ...
        # Calculate AST/TOV ratio
        ast_value = x[i]
        tov_value = y[i]
        ratio = ast_value / tov_value if tov_value > 0 else float('inf')
        
        # Create formatted text with only ratio in parentheses
        if ratio == float('inf'):
            player_text = f"{formatted_name} (Ratio: ∞)"
        else:
            player_text = f"{formatted_name} (Ratio: {ratio:.2f})"
        
        # Default position: closer to bubble (reduced distance)
        text_x = xi
        text_y = yi - bubble_radius_data - 0.01  # Reduced distance from 0.025 to 0.01
    ...
    # Label for vertical line (AST median) - positioned near x-axis
    ...
    # Label for horizontal line (TOV median) - positioned on y-axis
    ...
    # Bottom-left quadrant (Low AST, Low TOV)
    ax.text(x_min + padding_x, y_min + padding_y,
            'Rol secundario\npocas asistencias, pocas pérdidas',
            fontsize=10, fontweight='bold',
            ha='left', va='bottom',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='lightblue', alpha=0.7),
            color='darkblue')

    # Bottom-right quadrant (High AST, Low TOV)
    ax.text(x_max - padding_x, y_min + padding_y,
            'Creador élite\nmuchas asistencias, balón seguro',
            fontsize=10, fontweight='bold',
            ha='right', va='bottom',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='lightgreen', alpha=0.7),
            color='darkgreen')

    # Top-left quadrant (Low AST, High TOV)
    ax.text(x_min + padding_x, y_max - padding_y,
            'Zona de riesgo\npocas asistencias, muchas pérdidas',
            fontsize=10, fontweight='bold',
            ha='left', va='top',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='lightcoral', alpha=0.7),
            color='darkred')

    # Top-right quadrant (High AST, High TOV)
    ax.text(x_max - padding_x, y_max - padding_y,
            'Base con riesgo\nmuchas asistencias, muchas pérdidas',
            fontsize=10, fontweight='bold',
            ha='right', va='top',
    ...
    # 12) labels & title
    ax.set_xlabel('Asistencias por partido', fontsize=14)
    ax.set_ylabel('Pérdidas por partido', fontsize=14)
...
if __name__ == "__main__":
    ...
    fig = plot_top_assists_vs_turnovers(stats_df)
    fig.savefig("top_assists_vs_turnovers.png", dpi=300)
