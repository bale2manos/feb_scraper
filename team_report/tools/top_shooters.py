...
def plot_top_shooters(
    ...
    tcc_values = df['TCC']  # Assuming TCC is already computed in df
    ...
    for i, (xi, yi, formatted_name, team) in enumerate(zip(x, y, formatted_names, teams)):
        ...
        player_text = f"{formatted_name} ({tcc_value} TCC)"
    ...
    fig.suptitle(
        'Valoración del lanzamiento (mínimo 1 TCC)',
        fontsize=20,
        weight='bold',
...
if __name__ == "__main__":
    ...
    fig = plot_top_shooters(stats_df)
    fig.savefig("top_shooters.png", dpi=300)
