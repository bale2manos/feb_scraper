...
def plot_top_turnovers(
    ...
    tcc_values = df['TOV']  # Assuming TCC is already computed in df
    ...
    for i, (xi, yi, formatted_name, team) in enumerate(zip(x, y, formatted_names, teams)):
        ...
        # Get TCC value for this player
        tcc_value = int(tcc_values.iloc[i])

        # Create formatted text with TOV count in green
        player_text = f"{formatted_name} ({tcc_value} TOV)"
    ...
    # Bottom-left quadrant (Low Plays, Low TOV%)
    ax.text(x_min + padding_x, y_min + padding_y,
            'Poco uso\nbajo volumen, casi sin errores',
            fontsize=10, fontweight='bold',
            ha='left', va='bottom',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='lightblue', alpha=0.7),
            color='darkblue')

    # Bottom-right quadrant (High Plays, Low TOV%)
    ax.text(x_max - padding_x, y_min + padding_y,
            'Máquina de posesión\nalto volumen, balón seguro',
            fontsize=10, fontweight='bold',
            ha='right', va='bottom',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='lightgreen', alpha=0.7),
            color='darkgreen')

    # Top-left quadrant (Low Plays, High TOV%)
    ax.text(x_min + padding_x, y_max - padding_y,
            'Potencial errático\npocas posesiones, muchas pérdidas',
            fontsize=10, fontweight='bold',
            ha='left', va='top',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='lightcoral', alpha=0.7),
            color='darkred')

    # Top-right quadrant (High Plays, High TOV%)
    ax.text(x_max - padding_x, y_max - padding_y,
            'Uso extremo con riesgo\nmuchas posesiones, muchas pérdidas',
            fontsize=10, fontweight='bold',
            ha='right', va='top',
    ...
    ax.set_ylabel('TOV %', fontsize=14)
    # Main title and subtitle, both centered
    fig.suptitle(
        'Pérdidas por posesión (TO %)',
        fontsize=20,
        weight='bold',
...
if __name__ == "__main__":
    ...
    fig = plot_top_turnovers(stats_df)
    fig.savefig("top_turnovers.png", dpi=300)
