...
min_games: int = None
    ...
    """
    Genera una figura con los 4 jugadores que más puntos anotan de media (mínimo partidos configurables) para un equipo.
    ...
    df_team['PUNTOS_PJ'] = df_team['PUNTOS'] / df_team['PJ']
    top = df_team.sort_values('PUNTOS_PJ', ascending=False).head(4)
    ...
    for i, row in enumerate(top.itertuples()):
        ...
        ax.text(dorsal_x, y-0.04, f"{getattr(row, 'PUNTOS_PJ', 0):.1f} puntos", fontsize=10, weight='bold', color='#2E8B57', va='top', ha='left', fontfamily='Montserrat')
        if i < 3:
...
if __name__ == "__main__":
    ...
    fig.savefig('top_scorers_test.png', dpi=180, bbox_inches='tight')
    print("Gráfico de máximos anotadores guardado como 'top_scorers_test.png'")
